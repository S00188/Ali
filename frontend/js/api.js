...
(function () {
  ...
  async function request(method, path, body, { adminKey, hiddenKey, retries = 1 } = {}) {
    ...
    if (hiddenKey) headers["X-Hidden-Key"] = hiddenKey;
    ...
    if (retries > 0) {
      ...
      if (isNetworkError || isServerError) {
        ...
        return request(method, path, body, { adminKey, hiddenKey, retries: retries - 1 });
      }
    }
  ...
  const api = {
    ...
    // Yashirin (status="hidden") filmlar
    unlockHidden(password) {
      return post("/hidden/unlock", { password });
    },

    getHiddenMovies(hiddenKey, params = {}) {
      const q = new URLSearchParams();
      for (const [k, v] of Object.entries(params)) {
        if (v === undefined || v === null || v === "") continue;
        q.set(k, String(v));
      ...
      const qs = q.toString();
      return get("/movies/hidden" + (qs ? `?${qs}` : ""), { hiddenKey });
    },

    getMovieWithHiddenKey(id, hiddenKey) {
      return get(`/movies/${encodeURIComponent(id)}`, { hiddenKey });
    },

    adminGetHiddenPasswordStatus(key) {
      return get("/admin/hidden-password", { adminKey: key });
    },

    adminSetHiddenPassword(key, password) {
      return put("/admin/hidden-password", { password }, { adminKey: key });
    },
    ...
    getVideoUrl(id, quality, hiddenKey) {
      return get(`/movies/${encodeURIComponent(id)}/video/${encodeURIComponent(quality)}`, hiddenKey ? { hiddenKey } : undefined);
    },
