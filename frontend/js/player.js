      ...
      if (req) {
        ...
        try {
          ...
          if (result && typeof result.then === "function") {
            result.then(() => { handled = true; }).catch(() => {
              if (!handled) this._enterPseudoFullscreen();
            });
          }
        } catch (e) {
        ...
        // Ba'zi webview'lar (xususan Telegram) Promise qaytarmaydi va hech
        // qanday xato ham bermaydi — so'rov "muvaffaqiyatli" ko'rinadi-yu,
        // aslida hech narsa o'zgarmaydi. Shuning uchun qisqa fursatdan
        // keyin haqiqatan fullscreen bo'lganini tekshiramiz.
        setTimeout(() => {
          if (!handled && !this._inRealFullscreen(wrap) && !this.isPseudoFullscreen) {
            this._enterPseudoFullscreen();
          }
        }, 350);
        return;
