...
(function () {
  ...
  // Yashirin filmlar kaliti — admin kaliti bilan bir xil g'oya: foydalanuvchi
  // parolni bir marta kiritadi, to'g'ri bo'lsa lokalda saqlanadi va keyingi
  // barcha yashirin-film so'rovlarida X-Hidden-Key header orqali yuboriladi.
  let hiddenKey = "";
  try { hiddenKey = localStorage.getItem("kb_hidden_key") || ""; } catch (e) {}
  ...
  const state = {
    ...
    adminKey,
    hiddenKey,
    hiddenMovies: [],        // yashirin filmlar keshi (unlock qilingandan keyin)
    hiddenUnlocked: Boolean(hiddenKey),
    adminAuthed: false,
    adminTab: "films",
    adminStats: null,
  ...
  const DICT = {
    uz: {
      ...
      hiddenMoviesMenu: "Yashirin filmlar",
      hiddenMoviesTitle: "Yashirin filmlar",
      hiddenMoviesPrompt: "Ko'rish uchun parolni kiriting",
      hiddenMoviesPasswordPlaceholder: "Parol",
      hiddenMoviesUnlockBtn: "Ochish",
      hiddenMoviesEmpty: "Yashirin filmlar hali yo'q",
      hiddenPasswordModalTitle: "Yashirin filmlar paroli",
      hiddenPasswordConfiguredHint: "Parol o'rnatilgan. Yangi parol kiritsangiz, eskisi almashtiriladi.",
      hiddenPasswordNotConfiguredHint: "Parol hali o'rnatilmagan — o'rnatmaguningizcha yashirin filmlar bo'limi hech kimga ochilmaydi.",
      hiddenPasswordNew: "Yangi parol",
      hiddenPasswordPlaceholder: "Kamida 4 belgi",
      hiddenPasswordTooShort: "Parol kamida 4 belgidan iborat bo'lishi kerak",
    ...
    en: {
      ...
      hiddenMoviesMenu: "Hidden movies",
      hiddenMoviesTitle: "Hidden movies",
      hiddenMoviesPrompt: "Enter the password to view",
      hiddenMoviesPasswordPlaceholder: "Password",
      hiddenMoviesUnlockBtn: "Unlock",
      hiddenMoviesEmpty: "No hidden movies yet",
      hiddenPasswordModalTitle: "Hidden movies password",
      hiddenPasswordConfiguredHint: "A password is set. Entering a new one will replace it.",
      hiddenPasswordNotConfiguredHint: "No password set yet — the hidden movies section stays locked for everyone until you set one.",
      hiddenPasswordNew: "New password",
      hiddenPasswordPlaceholder: "At least 4 characters",
    ...
    ru: {
      ...
      hiddenMoviesMenu: "Скрытые фильмы",
      hiddenMoviesTitle: "Скрытые фильмы",
      hiddenMoviesPrompt: "Введите пароль для просмотра",
      hiddenMoviesPasswordPlaceholder: "Пароль",
      hiddenMoviesUnlockBtn: "Открыть",
      hiddenMoviesEmpty: "Скрытых фильмов пока нет",
      hiddenPasswordModalTitle: "Пароль скрытых фильмов",
      hiddenPasswordConfiguredHint: "Пароль установлен. Новый пароль заменит старый.",
      hiddenPasswordNotConfiguredHint: "Пароль ещё не установлен — раздел скрытых фильмов недоступен никому, пока вы его не зададите.",
      hiddenPasswordNew: "Новый пароль",
      hiddenPasswordPlaceholder: "Минимум 4 символа",
      hiddenPasswordTooShort: "Пароль должен содержать минимум 4 символа",
  ...
  function findMovie(id) {
    return state.movies.find((m) => m.id === id) ||
      state.hiddenMovies.find((m) => m.id === id) ||
      null;
  }
  ...
  async function loadData() {
    ...
    const [moviesRes, genresRes, favsRes, histRes, profRes, bannerRes, premiumRes, plansRes] = results;
  ...
  // Yashirin (status="hidden") filmlar ekrani — parol kiritilmagan/eskirgan
  // bo'lsa qulf ekranini, to'g'ri bo'lsa filmlar to'rini ko'rsatadi.
  async function renderHiddenMovies() {
    const lockedView = document.getElementById("hiddenLockedView");
    const listEl = document.getElementById("hiddenMoviesList");
    if (!state.hiddenKey) {
      lockedView.hidden = false;
      listEl.hidden = true;
      return;
    }
    const res = await KinoBotApi.getHiddenMovies(state.hiddenKey);
    if (!res.ok) {
      // Kalit noto'g'ri/eskirgan (admin parolni o'zgartirgan bo'lishi mumkin) —
      // qulflangan holatga qaytaramiz va saqlangan kalitni tozalaymiz.
      state.hiddenKey = "";
      try { localStorage.removeItem("kb_hidden_key"); } catch (e) {}
      lockedView.hidden = false;
      listEl.hidden = true;
      return;
    }
    state.hiddenMovies = res.data.movies || [];
    lockedView.hidden = true;
    listEl.hidden = false;
    listEl.innerHTML = state.hiddenMovies.map((m) => listCard(m)).join("") ||
      `<p class="search-empty">${t("hiddenMoviesEmpty")}</p>`;
  }
  ...
  function renderDetail(id) {
    ...
    // O'xshash filmlar backenddan (yashirin film bo'lsa X-Hidden-Key bilan)
    const similarReq = m.status === "hidden"
      ? KinoBotApi.getMovieWithHiddenKey(id, state.hiddenKey)
      : KinoBotApi.getMovie(id);
    similarReq.then((res) => {
  ...
  function renderPlayer(id) {
    ...
    const loaded = KinoBotPlayer.load(m, {
      ...
      // Yashirin (status="hidden") film bo'lsa, X-Hidden-Key ham yuboriladi.
      resolveUrl: (quality) =>
        KinoBotApi.getVideoUrl(m.id, quality, m.status === "hidden" ? state.hiddenKey : undefined)
          .then((json) => (json && json.ok ? json.data.url : null)),
    });
  ...
  async function renderAdmin() {
    ...
    const hiddenPwBtn = document.getElementById("adminHiddenPasswordBtn");
  ...
  function openPlanModal(planId = null) {
    ...
    if (isEdit) {
      const res = KinoBotApi.adminListPlans(state.adminKey); // will use cached in state?
      // We'll fetch fresh for edit
    }

    // For simplicity, we'll use a form modal
    const body = `
      <div class="plan-form">
        <div class="form-row">
    ...
    const modal = modalShell(isEdit ? t("planEditTitle") : t("planNewTitle"), body, {
      footer: `
        <button class="btn-ghost" data-dismiss>${t("cancelBtn")}</button>
            ...
            <div class="plan-multiselect" id="mf_genres">
              ${state.genres.map((g) => `
                <label class="plan-chip">
                  <input type="checkbox" value="${esc(g)}" ${Array.isArray(m.genres) && m.genres.includes(g) ? "checked" : ""}>
                  <span>${esc(g)}</span>
  ...
  // -- Admin modal: yashirin (status="hidden") filmlar paroli
  async function hiddenPasswordModal() {
    const res = await KinoBotApi.adminGetHiddenPasswordStatus(state.adminKey);
    const configured = res.ok && res.data.configured;
    const body = `
      <div class="form-grid" style="gap:16px;">
        <p class="premium-hint">${configured ? t("hiddenPasswordConfiguredHint") : t("hiddenPasswordNotConfiguredHint")}</p>
        <label class="span2" style="flex-direction:column; gap:8px;">
          <span style="display:flex; align-items:center; gap:8px;">${icon("key", 16)} ${t("hiddenPasswordNew")}</span>
          <input type="password" id="hp_new" autocomplete="new-password" placeholder="${t("hiddenPasswordPlaceholder")}" style="padding:14px 16px; font-size:15px;">
        </label>
      </div>`;
    const footer = `
      <button class="btn-secondary" data-modal-close>${t("cancelBtn")}</button>
      <button class="btn-primary" id="hp_save">${t("saveBtn")}</button>`;
    openModal(modalShell(t("hiddenPasswordModalTitle"), body, { footer: false }) +
      `<div class="modal-footer">${footer}</div>`);

    document.getElementById("hp_save").addEventListener("click", async () => {
      const password = document.getElementById("hp_new").value.trim();
      if (password.length < 4) { toast(t("hiddenPasswordTooShort")); return; }
      const btn = document.getElementById("hp_save");
      btn.disabled = true;
      const r = await KinoBotApi.adminSetHiddenPassword(state.adminKey, password);
      btn.disabled = false;
      if (r.ok) {
        toast(t("toastSaved"));
        closeModal();
  ...
  function bannerModal() {
    ...
    const bBody = `
      <div class="banner-manage">
        <div class="banner-head"><b>${t("bannerModalTitle")}</b></div>
        <div class="banner-type-sel" role="radiogroup" aria-label="${esc(t("bannerType"))}">
          <span class="banner-type-label">${esc(t("bannerType"))}</span>
          <button type="button" class="banner-type-btn ${type === "movie" ? "active" : ""}" role="radio" aria-checked="${type === "movie"}" data-btype="movie">${icon("film", 14)} ${esc(t("bannerTypeMovie"))}</button>
          <button type="button" class="banner-type-btn ${type === "ad" ? "active" : ""}" role="radio" aria-checked="${type === "ad"}" data-btype="ad">${icon("megaphone", 14)} ${esc(t("bannerTypeAd"))}</button>
        </div>

        <div class="banner-section bf-movie-only">
          <label class="form-field-label">${t("bannerMoviePick")}</label>
          <select id="bf_movieId" class="banner-select">
            ${movieOptions || `<option value="">${t("bannerEmpty")}</option>`}
          </select>
        </div>

        <div class="banner-section bf-ad-only">
          <label class="form-field-label">${t("bannerTitle")}</label>
          <input type="text" id="bf_title" class="banner-input" value="${esc(b.title || "")}" maxlength="200" placeholder="${t("bannerTitle")}">
        </div>

        <div class="banner-section bf-ad-only">
          <label class="form-field-label">${t("bannerText")}</label>
          <textarea id="bf_text" class="banner-textarea" rows="2" maxlength="500" placeholder="${t("bannerText")}">${esc(b.text || "")}</textarea>
        </div>

        <div class="banner-section bf-ad-only">
          <label class="form-field-label">${t("bannerLink")}</label>
          <input type="url" id="bf_link" class="banner-input" value="${esc(b.link || "")}" placeholder="https://..." maxlength="1000">
        </div>

        <div class="banner-section bf-ad-only">
          <label class="form-field-label">${t("bannerImage")}</label>
          <div class="banner-image-pick">
            <div class="banner-image-preview" id="bf_imagePreview">${preview}</div>
            <div class="banner-image-actions">
              <label class="btn-secondary btn-sm file-input-label">
                ${icon("upload", 13)} ${t("bannerImageUpload")}
                <input type="file" id="bf_imageFile" accept="image/*" hidden>
              </label>
        ...
        <div class="banner-section banner-active-row">
        ...
        <div class="banner-preview-box">
          <span class="banner-preview-label">${t("bannerPreview")}</span>
          <div class="banner-preview-frame" id="bfLivePreview">
            ${b.type === "ad"
              ? `<div class="banner-preview-ad">
                  ${b.imageUrl ? `<img src="${esc(b.imageUrl)}" alt="">` : ""}
                  <div class="banner-preview-ad-text">
                    <b>${esc(b.title || t("bannerAdDefaultTitle"))}</b>
                    <p>${esc(b.text || t("bannerAdDefaultText"))}</p>
                  </div>
                </div>`
              : `<div class="banner-preview-movie">
                  <div class="banner-preview-movie-title">${esc((state.movies.find((m) => m.id === b.movieId) || {}).title || t("bannerMoviePick"))}</div>
                </div>`}
          </div>
        </div>
      </div>`;
    ...
    openFsModal(fsModalShell(t("bannerModalTitle"), bBody, bFooter));

    // Turga qarab maydonlarni ko'rsatish/yashirish
    const typeBtns = document.querySelectorAll(".banner-type-btn");
    ...
    typeBtns.forEach((btn) => {
      btn.addEventListener("click", () => {
        typeBtns.forEach((x) => { x.classList.remove("active"); x.setAttribute("aria-checked", "false"); });
        btn.classList.add("active");
        btn.setAttribute("aria-checked", "true");
        toggleFields(btn.dataset.btype === "ad");
      });
    });

    function updateBannerPreview() {
      const isAd = document.querySelector(".banner-type-btn.active")?.dataset.btype === "ad";
      const preview = document.getElementById("bfLivePreview");
      if (!preview) return;
      if (isAd) {
        const title = document.getElementById("bf_title")?.value || t("bannerAdDefaultTitle");
        const text = document.getElementById("bf_text")?.value || t("bannerAdDefaultText");
        const imgSrc = document.getElementById("bf_imagePreview")?.querySelector("img")?.src || "";
        preview.innerHTML = `
          <div class="banner-preview-ad">
        ...
        const movieId = document.getElementById("bf_movieId")?.value;
        const movie = state.movies.find((m) => m.id === movieId);
        preview.innerHTML = `
          <div class="banner-preview-movie">
            <div class="banner-preview-movie-title">${esc(movie ? movie.title : t("bannerMoviePick"))}</div>
          </div>`;
      }
    }

    // Real-time preview updates
    document.getElementById("bf_title")?.addEventListener("input", updateBannerPreview);
    document.getElementById("bf_text")?.addEventListener("input", updateBannerPreview);
    document.getElementById("bf_movieId")?.addEventListener("change", updateBannerPreview);

    toggleFields(type === "ad");
  ...
  function autoExtractPoster(movie, quality) {
    return new Promise((resolve) => {
      ...
      KinoBotApi.getVideoUrl(movie.id, quality, movie.status === "hidden" ? state.hiddenKey : undefined).then((r) => {
  ...
  function openScreen(name, opts) {
    ...
    if (name === "hidden-movies") renderHiddenMovies();
  ...
  document.addEventListener("click", async (e) => {
    ...
    // Yashirin filmlar — parol bilan ochish
    if (e.target.closest("#hiddenUnlockBtn")) {
      const input = document.getElementById("hiddenPasswordInput");
      const password = input.value.trim();
      if (!password) return;
      const btn = document.getElementById("hiddenUnlockBtn");
      btn.disabled = true;
      const res = await KinoBotApi.unlockHidden(password);
      btn.disabled = false;
      if (res.ok && res.data.unlocked) {
        state.hiddenKey = password;
        try { localStorage.setItem("kb_hidden_key", password); } catch (e) {}
        input.value = "";
        renderHiddenMovies();
      } else {
        toast(res.error && res.error.message ? res.error.message : t("toastError"));
    ...
    if (e.target.closest("#adminHiddenPasswordBtn")) {
      hiddenPasswordModal();
      return;
    }
  ...
  function renderSubscribeChannelList(channels) {
    ...
    list.innerHTML = channels.map((c) => `
      <a class="subscribe-channel-btn" href="${esc(c.inviteLink || "#")}" target="_blank" rel="noopener">
  ...
  function showSubscribeOverlay(missingChannels) {
    ...
    checkBtn.onclick = async () => {
      ...
      if (res.ok && res.data.subscribed) {
        ...
        if (res.ok) renderSubscribeChannelList(res.data.missing);
      }
    };
  }
