...
const subscriptionCache = new Map(); // userId -> { missing: [...], expiresAt }
...
async function checkSubscription(userId) {
  ...
  if (cached && cached.expiresAt > Date.now()) {
    return { subscribed: cached.missing.length === 0, missing: cached.missing };
  }
  ...
  subscriptionCache.set(String(userId), { missing, expiresAt: Date.now() + SUBSCRIPTION_CACHE_TTL_MS });
...
// -- "Yashirin" (status="hidden") filmlar uchun kalit tekshiruvi --
// Admin panelning X-Admin-Key ishlatilishi kabi mantiq: foydalanuvchi bir
// marta parolni to'g'ri kiritadi (POST /api/hidden/unlock), frontend uni
// lokalda saqlaydi va keyingi barcha yashirin-film so'rovlarida
// X-Hidden-Key header orqali yuboradi. Parol o'rnatilmagan bo'lsa (admin
// hali sozlamagan) — hech kim kira olmaydi (fail-closed).
function verifyHiddenKey(req) {
  const hash = repos.settings.getHiddenMoviesPasswordHash();
  if (!hash) return false;
  const key = req.headers["x-hidden-key"];
...
async function handleMovieDetail(req, res, id) {
  ...
  // "inactive" — hech kimga ko'rinmaydi (qoralama). "hidden" — faqat to'g'ri
  // X-Hidden-Key bilan. Mavjudligini oshkor qilmaslik uchun ikkalasida ham
  // xuddi shu 404 qaytariladi.
  if (movie.status === "inactive") return fail(res, 404, "NOT_FOUND", "Film topilmadi");
  if (movie.status === "hidden" && !verifyHiddenKey(req)) return fail(res, 404, "NOT_FOUND", "Film topilmadi");
...
// -- Yashirin (status="hidden") filmlar (public, parol bilan) ----

// POST /api/hidden/unlock — parolni tekshiradi (frontend to'g'ri bo'lsa
// kalitni lokalda saqlaydi va keyingi so'rovlarda X-Hidden-Key yuboradi).
async function handleHiddenUnlock(req, res) {
  const rl = applyRateLimit(req, res, limitAuth, rateLimitKey(req));
  if (!rl.allowed) return fail(res, 429, "RATE_LIMITED", "Juda ko'p urinish, birozdan so'ng qayta urinib ko'ring");

  const hash = repos.settings.getHiddenMoviesPasswordHash();
  if (!hash) return fail(res, 503, "CONFIG_ERROR", "Yashirin filmlar paroli hali sozlanmagan");

  let body;
  try { body = await readBody(req); } catch (e) { return fail(res, 400, "BAD_REQUEST", e.message); }
  const password = body && typeof body.password === "string" ? body.password : "";
  if (!password) return fail(res, 422, "VALIDATION_ERROR", "Parol majburiy");

  if (!verifyPassword(password, hash)) {
    return fail(res, 401, "WRONG_PASSWORD", "Parol noto'g'ri");
  }
  ok(res, 200, { unlocked: true });
}

// GET /api/movies/hidden — X-Hidden-Key to'g'ri bo'lsa yashirin filmlar ro'yxati.
async function handleHiddenMovies(req, res, url) {
  if (!verifyHiddenKey(req)) {
    return fail(res, 403, "FORBIDDEN", "Yashirin filmlarni ko'rish uchun avval parolni kiriting");
  }
  const result = repos.movies.list({
    status: "hidden",
    genre: url.searchParams.get("genre"),
...
// -- Admin: yashirin filmlar paroli ----
async function handleAdminGetHiddenPassword(req, res) {
  const admin = requireAdmin(req, res);
  if (!admin) return;
  ok(res, 200, { configured: Boolean(repos.settings.getHiddenMoviesPasswordHash()) });
}

async function handleAdminSetHiddenPassword(req, res) {
  const admin = requireAdmin(req, res);
  if (!admin) return;
  ...
  if (password.length < 4) {
    return fail(res, 422, "VALIDATION_ERROR", "Parol kamida 4 belgidan iborat bo'lishi kerak");
  }

  await repos.settings.setHiddenMoviesPasswordHash(hashPassword(password));
  await logAudit({ adminId: admin.userId, action: "ADMIN_SET_HIDDEN_PASSWORD", entityType: "settings", entityId: "hiddenMoviesPassword" });
  ok(res, 200, { configured: true });
}
...
async function handleMovieVideoUrl(req, res, id, quality) {
  ...
  // Yashirin (status="hidden") film — to'g'ri X-Hidden-Key bo'lmasa video
  // berilmaydi (mavjudligini oshkor qilmaslik uchun 404, 403 emas).
  // "inactive" — hech kimga, hatto parol bilan ham.
  if (movie.status === "inactive") return fail(res, 404, "NOT_FOUND", "Film topilmadi");
  if (movie.status === "hidden" && !verifyHiddenKey(req)) {
    return fail(res, 404, "NOT_FOUND", "Film topilmadi");
  }
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    // GET /api/movies/hidden — yashirin filmlar ro'yxati (X-Hidden-Key kerak).
    // MUHIM: bu route /movies/:id (pastda) dan OLDIN bo'lishi shart, aks
    // holda "hidden" so'zi ID sifatida qabul qilinib ketadi.
    if (req.method === "GET" && parts[0] === "api" && parts[1] === "movies" && parts[2] === "hidden" && !parts[3]) {
      return handleHiddenMovies(req, res, url);
    }
    ...
    // POST /api/hidden/unlock — yashirin filmlar paroli tekshiruvi
    if (req.method === "POST" && parts[0] === "api" && parts[1] === "hidden" && parts[2] === "unlock") {
      return handleHiddenUnlock(req, res);
    }
    ...
    // GET /api/admin/hidden-password — yashirin filmlar paroli o'rnatilganmi?
    if (req.method === "GET" && parts[0] === "api" && parts[1] === "admin" && parts[2] === "hidden-password") {
      return handleAdminGetHiddenPassword(req, res);
    }
    // PUT /api/admin/hidden-password — parolni o'rnatish/o'zgartirish
    if (req.method === "PUT" && parts[0] === "api" && parts[1] === "admin" && parts[2] === "hidden-password") {
      return handleAdminSetHiddenPassword(req, res);
    }
